...
export default function ContentBlock() {
  ...
  useEffect(() => {
    ...
    const letters = contentBlockTitleRef && contentBlockTitleRef.current && contentBlockTitleRef.current.querySelectorAll('span');
    tl.from(letters, {
      opacity: 0,
      y: -50,
      duration: 0.75,
    ...
    if (contentBlockContentRef.current?.children) {
      tl.from(
        Array.from(contentBlockContentRef.current.children),
        { opacity: 0, y: 50, duration: 0.75, stagger: 0.25, ease: 'power2.out' }
      );
    }
  }, []);
          ...
          <div className={style.coverPage}>
            <div className={style.contentParagraph}>
              ...
              <div ref={contentBlockContentRef} className={style.content}>
                <p>
                  Descubre Pacific Waves Hostel & Surf, un rincón acogedor a orillas
                  del majestuoso océano Pacífico, donde cada amanecer trae consigo
                  la promesa de nuevas aventuras. Situado en la impresionante Playa
                  El Almejal, nuestro hostal combina la serenidad de la naturaleza
                  con la energía del surf y el espíritu libre del viajero moderno.
                </p>
                <p>
                  Sumérgete en una experiencia única que va más allá del
                  alojamiento. Aquí no solo dormirás con el sonido de las olas:
                  podrás vivir cada día intensamente entre clases de surf para todos
                  los niveles, excursiones a cascadas escondidas, caminatas por
                  senderos tropicales y, por supuesto, el asombroso espectáculo del
                  avistamiento de ballenas —todo desde la comodidad de nuestro
                  entorno frente al mar.
                </p>
              </div>
            </div>
